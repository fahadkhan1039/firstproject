...
<script type="text/javascript">
    var persnalities_table = $("#all_users").DataTable({
        serverSide: true,
        processing: true,
        "ajax": {
            "url": '{{ route('view-all-user') }}',
            "type": "GET",
            "dataSrc": function (json) {
                // Check if the data is empty
                if (json.data.length === 0) {
                    // You can display a custom message in the table when no data is returned
                    $('#all_users tbody').html('<tr><td colspan="5" class="text-center">No data available</td></tr>');
                }
                return json.data;
            },
            "error": function (xhr, status, error) {
                // Handle AJAX error
                console.log(error);
                $('#all_users tbody').html('<tr><td colspan="5" class="text-center">Error loading data</td></tr>');
            }
        },
        columns: [
            {data: 'id', name: 'id'},
        ...
        "createdRow": function(row, data, dataIndex) {
            var start = persnalities_table.page.info().start;
